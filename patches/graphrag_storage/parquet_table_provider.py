...
class ParquetTableProvider(TableProvider):
    ...
    async def read_dataframe(self, table_name: str) -> pd.DataFrame:
        ...
        Notes
        -----
        Implementation uses ``pq.ParquetFile.iter_batches()`` rather than
        ``pd.read_parquet()`` to work around a PyArrow >=22.0 bug where
        ``ChunkedArray``-to-pandas conversion raises
        ``ArrowNotImplementedError: Nested data conversions not implemented for
        chunked array outputs`` for files with nested column types (e.g.
        ``list<string>``) that span multiple row groups.  ``iter_batches()``
        yields ``RecordBatch`` objects (non-chunked), which convert correctly.
        ...
        try:
            ...
            data = BytesIO(await self._storage.get(filename, as_bytes=True))
            # Work around PyArrow >=22 bug: ChunkedArray-to-pandas fails for
            # nested types (e.g. list<string>) in multi-row-group files.
            # iter_batches() yields RecordBatch (non-chunked) which converts fine.
