...
.USAGE
    *** RE-RUN AFTER CRASH / INTERRUPTION: pass NO flags. Ever. ***
    ...
    FORCE A STEP TO RE-RUN (delete its marker, then plain re-run):
      Re-convert : Remove-Item "D:\FinanceRAG-cloud\.convert_done.json"
      Re-index   : Remove-Item "D:\FinanceRAG-cloud\.shard_status.json"; Remove-Item "D:\FinanceRAG-cloud\output" -Recurse
