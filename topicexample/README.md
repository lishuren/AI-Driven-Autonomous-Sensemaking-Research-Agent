...
├── resources/               ← plain-text notes, Markdown, CSV, logs (complex formats: run prepare scripts first)
│   └── README.md
├── graphrag/                ← GraphRAG indexed output (generated by prepare script)
│   └── .gitignore
├── output/                  ← created automatically; all run artifacts go here
├── prepare.ps1              ← Windows: index resources into graphrag/
├── prepare.sh               ← Linux/macOS: index resources into graphrag/
...
|------------|--------------|
| `requirements.md` | Parsed for `## Topic` (query), `## Research Focus`, `## Background`, and `## Constraints` |
| `prompts/` | Overrides the bundled prompt templates |
| `resources/` | Plain-text files (`.md`, `.txt`, `.csv`, etc.) are read directly by the agent; complex formats (PDF, DOCX, source code) must be indexed first via `prepare.ps1` / `prepare.sh` → `graphrag/` |
| `graphrag/` | Pre-built GraphRAG index; the Scout queries it alongside live web search |
...
Place plain-text notes, Markdown, CSV, or log files in `resources/` —
the agent reads them directly on each iteration.

For complex formats (PDFs, Word documents, source code, etc.), index them
into a GraphRAG knowledge graph first:
...
This requires the `graphragloader` package and an LLM + embedding model
(Ollama by default).  See `graphragloader/README.md` for setup details.
...
| `graph_viewer.html` | Open in a browser — interactive HTML graph viewer with local resource provenance |
