...
├── resources/               ← local PDFs, Word docs, source code, text files
│   └── README.md
...
| `resources/` | Local documents (PDF, DOCX, source code, Markdown, text) are indexed into a GraphRAG knowledge graph via `prepare.ps1` / `prepare.sh` |
...
Place any PDFs, Word documents, source code, or text files in `resources/`.

Then index them into a GraphRAG knowledge graph:
